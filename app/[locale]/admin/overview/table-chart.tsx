          ...
          {image ? (
            <Link className='flex items-end ' href={`/admin/products/${id}`}>
              <Image
                ...
                height={36}
              />
              <p className='text-center text-xm whitespace-nowrap overflow-hidden text-ellipsis'>
                {label}
              </p>
            </Link>
          ) : (
            <div className='flex items-end text-xm'>{label}</div>
          )}

          <ProgressBar value={percentage} />

          <div className='text-xm text-right flex items-center'>
